...
def test_stderr():
    suites = serialize_and_read(
        Suite(name='test', stderr='I am stderr!', test_cases=[Case(name='Test1')]))[0]
    assert suites[0].getElementsByTagName('system-err')[0].firstChild.data == 'I am stderr!'


def test_stdout_stderr():
    suites = serialize_and_read(
        Suite(name='test', stdout='I am stdout!', stderr='I am stderr!', test_cases=[Case(name='Test1')]))[0]
    assert suites[0].getElementsByTagName('system-err')[0].firstChild.data == 'I am stderr!'
...
def test_no_assertions():
    suites = serialize_and_read(
        Suite(name='test', test_cases=[Case(name='Test1')]))[0]
    assert not suites[0].getElementsByTagName('testcase')[0].hasAttribute('assertions')


def test_assertions():
    suites = serialize_and_read(
        Suite(name='test', test_cases=[Case(name='Test1', assertions=5)]))[0]
