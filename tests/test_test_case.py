...
def test_init_classname_time_timestamp():
    ts, tcs = serialize_and_read(Suite(
        'test',
        [Case(name='Test1', classname='some.class.name', elapsed_sec=123.345, timestamp=99999)]
    ))[0]
    verify_test_case(
        tcs[0],
        {'name': 'Test1', 'classname': 'some.class.name', 'time': ("%f" % 123.345), 'timestamp': ("%s" % 99999)}
    )


def test_init_stderr():
    ts, tcs = serialize_and_read(Suite(
        'test', [Case(name='Test1', classname='some.class.name', elapsed_sec=123.345, stderr='I am stderr!')]
    ))[0]
    verify_test_case(
        tcs[0],
...
def test_init_failure_type():
    ...
    verify_test_case(
        tcs[0],
        {'name': 'Failure-Type'}, failure_message="failure message", failure_type='com.example.Error'
    )
